...
module ActiveRDF
  class JenaAdapter < ActiveRdfAdapter
    ...
    class DataSourceDBConnection < Jena::SDB::SDBConnection
    ...
    def initialize(params = {})
      ...
      if dbparams
        if dbparams[:datasource]
          ...
          begin
            if !Jena::SDB.send("#{dbparams[:type].downcase}_available?")
          ...
          self.connection = Jena::SDB::SDBConnection.new(dbparams[:url],
            dbparams[:username],
            dbparams[:password],
            dbparams[:type])
        end
      ...
      elsif params[:tdb] then
        self.root_directory = params[:tdb]
        self.dataset = Jena::TDB::TDBFactory.createDataset(self.root_directory)
        self.base_model = self.dataset.getDefaultModel()
        self.model_maker = Jena::Model::ModelFactory.createMemModelMaker
      else
        self.model_maker = Jena::Model::ModelFactory.createMemModelMaker
      end

      if self.base_model.nil?
    ...
    def build_object(object, submodel = nil)
      ...
      if object.respond_to? :uri
        ...
        if !object.kind_of? RDFS::Literal
          objlit = RDFS::Literal.new object #XXX - Literal is now module
        else
          objlit = object
        end

        if objlit.respond_to? :type
          type = Jena::Datatypes::TypeMapper.getInstance.getTypeByName(objlit.type.uri)
          o = mod.createTypedLiteral(objlit.value, type)
        elsif objlit.respond_to? :language
          o = mod.createLiteral(objlit.value, objlit.language)
        elsif objlit.kind_of? RDFS::Literal
          o = mod.createLiteral(objlit.to_literal_s)
        else
    ...
    def query_pellet(query)
      ...
      qexec = Pellet::Sparqldl::Jena::SparqlDLExecutionFactory.create(jena_query, self.model)
    ...
    #FIXME - Remove once 'perform_query' stabilizes. This is just a reference.
    def perform_query0(query, qexec)
      results = qexec.execSelect
      arr_results = []
        ...
        query.select_clauses.each do |kw|
          ...
          if thing.kind_of? Jena::Model::Resource
            ...
            if thing.getLanguage == "" and thing.getDatatypeURI.nil?
              ...
              # datatyped literal
              res_row << RDFS::Literal.new(thing.getValue, RDFS::Resource.new(thing.getDatatypeURI))
            elsif thing.getDatatypeURI.nil?
              # language tagged literal
              res_row << RDFS::Literal.new(thing.getLexicalForm, "@" + thing.getLanguage)
            else
              raise ActiveRdfError, "Jena Sparql returned a strange literal"
            end
          else
